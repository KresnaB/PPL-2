...
void movementpemain(char gerak, game arr[BRS][KLM], int baris_, int kolom_)
{
    switch(gerak){
    case 'A' :
            setviewport(((kolom_)*MATRIX), ((baris_)*MATRIX),((kolom_+2)*MATRIX)+1, ((baris_+1)*MATRIX)+1,1);
            clearviewport();
    ...
    case 'D' :
            ...
            break;
    case 'W' :
            setviewport((kolom_*MATRIX), ((baris_)*MATRIX),((kolom_+1)*MATRIX)+1, ((baris_+2)*MATRIX)+1,1);
            clearviewport();
